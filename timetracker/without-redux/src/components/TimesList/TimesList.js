...
class TimesList extends Component {
    ...
    return (
      <div className="TimesList">
        <table>
          ...
          <tfoot>{this.renderSummary()}</tfoot>
        </table>
  ...
  renderListItems() {
    ...
    return times.map(this.calculateDuration).map((timeEntry, index) => (
  ...
  renderSummary() {
    ...
    const sum = times
      ...
      .reduce(
        (sum, timeEntry) => sum.add(timeEntry.duration),
        Moment.duration(0)
      );
    ...
    return (
      <div className="TimesList-form">
        ...
        <form onSubmit={this.handleSubmit}>
          ...
          <input
            name="inputFromTime"
            value={this.state.inputFromTime}
            type="time"
